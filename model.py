...
def set_dropout_mode(model, mode=True):
    """
    递归地设置模型中所有 Dropout 层的模式 (train 或 eval)。
    同时确保其他层（如 BatchNorm）保持在 eval 模式。
    """
    for module in model.modules():
        if isinstance(module, nn.Dropout):
            if mode:
                module.train() # 开启 Dropout
            else:
                module.eval() # 关闭 Dropout (虽然 eval() 模式默认关闭，显式调用更清晰)
        elif not isinstance(module, nn.Dropout) and hasattr(module, 'eval'):
             # 对于非 Dropout 层，确保它们在 eval 模式
             # 这对于 BatchNorm 等层在 MC Dropout 期间保持统计数据不变很重要
             module.eval()

def predict_with_mc_dropout(model, data_loader, device, mc_samples=50):
    """
    使用蒙特卡洛 Dropout 进行预测，估计模型不确定性。

    参数:
        model: 训练好的模型。
        data_loader: 测试数据加载器。
        device: 计算设备。
        mc_samples (int): MC Dropout 的采样次数。

    返回:
        tuple: (mean_predictions, uncertainties, all_targets)
               - mean_predictions: 每个样本的平均预测值 (numpy array)。
               - uncertainties: 每个样本预测的标准差 (不确定性) (numpy array)。
               - all_targets: 对应的真实标签 (numpy array)。
    """
    logger.info(f"开始使用 MC Dropout 进行预测，采样次数: {mc_samples}")
    
    # 关键：设置模型为评估模式，但手动开启 Dropout 层
    model.eval() # 先整体设为 eval
    set_dropout_mode(model, mode=True) # 再单独开启 Dropout

    all_mc_predictions = [] # 存储所有 MC 样本的预测结果 [mc_samples, num_total_samples]
    all_targets_list = []   # 存储所有真实标签

    # 为每个 MC 样本进行一次完整的预测
    for i in tqdm(range(mc_samples), desc="MC Dropout Sampling"):
        predictions_one_sample = []
        targets_one_sample = [] # 只需要收集一次目标值
        
        with torch.no_grad(): # 预测时不需要计算梯度
            for data, targets in data_loader:
                data = move_to_device(data, device)
                
                # 使用 autocast (如果配置了混合精度)
                use_amp = hasattr(config.training_config, 'use_amp') and config.training_config['use_amp']
                ...
                predictions_one_sample.append(outputs.cpu().numpy())
                if i == 0: # 只需要在第一次 MC 采样时收集目标值
                    targets_one_sample.append(targets.cpu().numpy())

        all_mc_predictions.append(np.concatenate(predictions_one_sample).flatten())
        if i == 0:
            all_targets_list = np.concatenate(targets_one_sample).flatten()

    # 恢复模型的标准评估模式
    model.eval()

    # 转换为 numpy 数组 [mc_samples, num_total_samples]
    all_mc_predictions = np.array(all_mc_predictions)

    # 计算平均预测值和不确定性 (标准差)
    mean_predictions = np.mean(all_mc_predictions, axis=0)
    uncertainties = np.std(all_mc_predictions, axis=0)

    logger.info(f"MC Dropout 预测完成。平均预测值 shape: {mean_predictions.shape}, 不确定性 shape: {uncertainties.shape}")
    
    return mean_predictions, uncertainties, all_targets_list